...
void solve()
{
    ...
    f(i, 0, n)
    {
        if (i == 0)
        {
            cout << abs(a[i + 1] - a[i]) << " " << abs(a[n - 1] - a[i]) << ee;
        }
        else if (i == (n - 1))
        {
            cout << abs(a[i] - a[i - 1]) << " " << abs(a[i] - a[0]) << ee;
        }
        else
        {
            cout << min(min(abs(a[i] - a[i - 1]), abs(a[0] - a[i])), min(abs(a[i + 1] - a[i]), abs(a[i] - a[0]))) << " " << max(max(abs(a[i] - a[i - 1]), abs(a[0] - a[i])), max(abs(a[i + 1] - a[i]), abs(a[n - 1] - a[i]))) << ee;
        }
    }
}
